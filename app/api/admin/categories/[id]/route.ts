...
import { categoryDetailQuerySchema, categoryUpdateSchema } from '@/lib/validators/category'
import { serializeCategory, productWithRelations } from '@/lib/serializers'
...
export async function GET(request: NextRequest, ctx: RouteCtx) {
  ...
  try {
    ...
    if (!category) {
      return NextResponse.json({ error: 'Fant ikke kategori.' }, { status: 404 })
    }
...
export async function PUT(request: NextRequest, ctx: RouteCtx) {
  ...
  try {
    ...
    if (data.parentId && data.parentId === id) {
      return NextResponse.json({ error: 'En kategori kan ikke være sin egen forelder.' }, { status: 400 })
    }
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      ...
      if (error.code === 'P2002') {
        return NextResponse.json({ error: 'Slug må være unik.' }, { status: 409 })
      }
    }
    return handleError(error)
  }
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      ...
      if (error.code === 'P2003') {
        return NextResponse.json(
          {
            error: 'Kan ikke slette kategori som fortsatt er i bruk av andre ressurser.',
          },
          { status: 409 }
        )
      }
