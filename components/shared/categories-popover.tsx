  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent
        align="start"
        sideOffset={16}
        className="w-full md:w-[min(95vw,1100px)] max-h-[calc(100vh-100px)] md:max-h-[calc(100vh-150px)] p-0 border-border/80 bg-background/95 backdrop-blur-lg shadow-2xl flex flex-col overflow-hidden"
      >
        <div className="flex flex-col md:grid md:grid-cols-[280px,1fr] overflow-y-auto min-h-0">
          <aside className="p-3 md:p-5 border-b md:border-b-0 md:border-r border-border/60 space-y-3 md:space-y-5">
            <section>
            ...
            {loading && !categoriesWithProducts.length ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 md:gap-3">
                {Array.from({ length: 10 }).map((_, index) => (
                  <Skeleton key={index} className="h-14 md:h-16 rounded-lg" />
                ))}
              </div>
            ) : (
